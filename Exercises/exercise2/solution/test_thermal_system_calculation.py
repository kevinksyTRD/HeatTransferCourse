...
def test_get_maximum_velocity_for_tube_bank_configuration(velocity_random):
    ...
    for arrangement_to_test in TubeBankArrangement:
        tube_bank_config_narrow = TubeBankConfiguration(
            ...
            number_tubes_each_row=random.randint(5, 10)
        )
...
def tube_bank_configuration_random() -> TubeBankConfiguration:
    ...
    return TubeBankConfiguration(
        ...
        number_tubes_each_row=random.randint(5, 20)
    )
...
def test_forced_convection_tube_banks_cross_flow(
    ...
    fluid_state_inlet = FluidState(
        fluid=Fluid[convection.fluid_state.fluid],
        temp_k=convection.temp_infinity,
        characteristic_length=convection.fluid_state.characteristic_length
    )
    temp_out_answer = convection.temp_surface - \
        (convection.temp_surface - convection.temp_infinity) * np.exp(
            -1 * (np.pi * convection.fluid_state.characteristic_length *
                  convection.configuration.number_tubes_total * convection.h) /
            (fluid_state_inlet.density * convection.velocity *
             convection.configuration.number_tubes_each_row *
             convection.configuration.vertical_spacing *
             fluid_state_inlet.cp)
        )
    assert np.isclose(convection.temp_out, temp_out_answer), "The outlet temperature estimation " \
                                                             "is wrong"

    # Test log_mean_temperature_difference
    lmtd_answer = (convection.temp_out - convection.temp_infinity) / \
        np.log((convection.temp_surface - convection.temp_infinity) /
               (convection.temp_surface - convection.temp_out))
    assert np.isclose(convection.log_mean_temperature_difference, lmtd_answer), \
        "The value of the log mean of temperature difference is wrong."

    # Test get_heat_transfer_rate
